...
jobs:
  build:
    ...
    steps:
      - id: aws-auth
        uses: khulnasoft/shared-workflows/actions/aws-auth@aws-auth-v1.0.1
        with:
      ...
      - id: cat-file-from-s3-bucket
        run: |
          aws s3 cp 's3://khulnasoftlabs-github-actions-test-repo/test.txt' 'test.txt'
          cat 'test.txt'
```
...
As this defines which GitHub Actions runs are allowed to use the role's permissions, it is critical to make these configurations as precise as possible. Furthermore, all runs are limited to be triggered exclusively from repositories under `khulnasoft/`, and it is not possible to exceed this restriction.
  ...
  "Statement": [
    {
      ...
      "Condition": {
        "StringEquals": {
          "aws:PrincipalTag/job_workflow_ref": "khulnasoft/<REPO>/.github/workflows/<WORKFLOW_FILE>@refs/heads/main"
        }
      }
    }
  ]
  ...
  "Statement": [
    {
      "Effect": "Allow",
      "Action": ["s3:GetObject"],
      "Resource": "arn:aws:s3:::khulnasoftlabs-github-actions-${aws:PrincipalTag/repository_name}/*"
    }
  ]
}
```
